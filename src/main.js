
/*import './style.css'
import javascriptLogo from './javascript.svg'
import viteLogo from '/vite.svg'
import { setupCounter } from './counter.js'

// 🌍 --- FONDOS ESTACIONALES / FESTIVOS --- 🌍
const today = new Date();
const month = today.getMonth(); // Enero = 0
const day = today.getDate();

let backgroundImage = '/business-office.jpg'; // Fondo por defecto
let themeClass = ''; // Clase opcional para CSS animado

// 🎃 HALLOWEEN (29–31 octubre)
if (month === 9 && day >= 23 && day <= 31) {
  backgroundImage = '/halloween2.jpg';
  themeClass = 'halloween-mode';
}

// 🎄 NAVIDAD (10–31 diciembre)
else if (month === 11 && day >= 10 && day <= 31) {
  backgroundImage = '/christmas.jpg';
  themeClass = 'christmas-mode';
}

// 🎆 AÑO NUEVO (1–7 enero)
else if (month === 0 && day >= 1 && day <= 7) {
  backgroundImage = '/añonuevo.jpg';
  themeClass = 'newyear-mode';
}

// 🌞 VERANO (15 junio – 31 agosto)
else if ((month === 5 && day >= 15) || (month > 5 && month < 8) || (month === 7 && day <= 31)) {
  backgroundImage = '/verano.jpg';
  themeClass = 'summer-mode';
}

// Aplica la clase temática al body (para estilos opcionales)
if (themeClass) document.body.classList.add(themeClass);

// --- CONTADORES DE VISITAS ---
const todayStr = new Date().toISOString().slice(0, 10); // yyyy-mm-dd
const week = (() => {
  const d = new Date();
  const firstDay = new Date(d.setDate(d.getDate() - d.getDay())); // lunes
  return firstDay.toISOString().slice(0, 10);
})();

// Inicializar contadores en localStorage
let visitasHoyData = JSON.parse(localStorage.getItem('visitasHoyData') || '{}');
let visitasSemanaData = JSON.parse(localStorage.getItem('visitasSemanaData') || '{}');

// Total
let totalVisits = parseInt(localStorage.getItem('totalVisits') || '0') + 1;
localStorage.setItem('totalVisits', totalVisits);

// Hoy y Semana
visitasHoyData[todayStr] = (visitasHoyData[todayStr] || 0) + 1;
visitasSemanaData[week] = (visitasSemanaData[week] || 0) + 1;

localStorage.setItem('visitasHoyData', JSON.stringify(visitasHoyData));
localStorage.setItem('visitasSemanaData', JSON.stringify(visitasSemanaData));

const visitasHoy = visitasHoyData[todayStr];
const visitasSemana = visitasSemanaData[week];

// --- HTML ---
document.querySelector('#app').innerHTML = `
  <div class="bg-cover bg-center h-screen flex flex-col justify-center ${themeClass}"
       style="background-image: url(${backgroundImage});">

    <!-- Nombre -->
    <div class="rounded-sm flex flex-col text-center">
      <h1 class="bg-white/25 ml-2 mr-2 text-2xl p-1 mt-1 rounded-xl text-emerald-50 font-bold flex flex-col">Andrea Hernández</h1>
    </div>

    <!-- Títulos -->
    <h1 class="ml-2 mr-2 mt-6 text-xl bg-white/15 p-2 rounded-xl text-violet-50 text-center">
      <strong>Programadora Web Frontend</strong>
    </h1>
    <h1 class="ml-2 mr-2 mt-1 text-xs p-1 rounded-xl text-emerald-50 text-center">
      <strong>Licenciada en Pedagogia General de la Música <br> Conservatorio Superior del Liceo</strong>
    </h1>

    <!-- Enlaces sociales -->
    <div class="flex justify-center justify-items-center mt-2 mb-5">
      <!-- TikTok, GitHub, LinkedIn -->
      <a href="https://www.tiktok.com/@fioripoeticblog?lang=es-419">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          xmlns="http://www.w3.org/2000/svg" width="44px" height="44px" viewBox="0 0 48 48" fill="#ffffff">
          <path d="M33.5 12.7a9.3 9.3 0 0 1-3.2-7.1h-5.1v24a4.4 4.4 0 1 1-4.4-4.4
          4.4 4.4 0 0 1 1.4.2v-5.2a9.6 9.6 0 0 0-1.4-.1 9.6 9.6 0 1 0 9.6 9.6v-12
          a14.3 14.3 0 0 0 8.3 2.7V15a9.2 9.2 0 0 1-5.2-2.3z"/>
        </svg>
      </a>
      <a href="https://github.com/81andie">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          width="44px" height="44px" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" fill="#ffffff" stroke="#ffffff">
          <defs>
            <style>
              .cls-1 { fill: none; stroke: #ffffff; stroke-linecap: round; stroke-linejoin: round; fill-rule: evenodd; }
            </style>
          </defs>
          <path class="cls-1"
            d="M24,2.5a21.5,21.5,0,0,0-6.8,41.9c1.08.2,1.47-.46,1.47-1s0-1.86,0-3.65
            c-6,1.3-7.24-2.88-7.24-2.88A5.7,5.7,0,0,0,9,33.68c-1.95-1.33.15-1.31.15-1.31
            a4.52,4.52,0,0,1,3.29,2.22c1.92,3.29,5,2.34,6.26,1.79a4.61,4.61,0,0,1,1.37-2.88
            c-4.78-.54-9.8-2.38-9.8-10.62a8.29,8.29,0,0,1,2.22-5.77,7.68,7.68,0,0,1,.21-5.69
            s1.8-.58,5.91,2.2a20.46,20.46,0,0,1,10.76,0c4.11-2.78,5.91-2.2,5.91-2.2
            a7.74,7.74,0,0,1,.21,5.69,8.28,8.28,0,0,1,2.21,5.77
            c0,8.26-5,10.07-9.81,10.61a5.12,5.12,0,0,1,1.46,4
            c0,2.87,0,5.19,0,5.9s.39,1.24,1.48,1A21.5,21.5,0,0,0,24,2.5"/>
        </svg>
      </a>
      <a href="https://www.linkedin.com/in/andrea-hern%C3%A0ndez-abbab9288">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          xmlns="http://www.w3.org/2000/svg" width="44px" height="44px" viewBox="0 0 48 48" fill="#ffffff">
          <path d="M12 19h5.3v17H12zM14.6 12a3.1 3.1 0 1 1 0 6.2 3.1 3.1 0 0 1 0-6.2zM20.6 19h5v2.3
            h.1a5.5 5.5 0 0 1 5-2.8c5.3 0 6.3 3.5 6.3 8v9.5h-5.3v-8.4c0-2-.1-4.6-2.8-4.6
            s-3.3 2.1-3.3 4.4v8.6h-5.3V19z"/>
        </svg>
      </a>
    </div>

    <!-- MINI BADGE CON TOTAL / HOY / SEMANA -->
      <span class="text-center text-white"><strong>Visitas</strong></span>
  
    <div class="flex justify-center mt-4 space-x-1 text-white text-sm">
  
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Total: <strong>${totalVisits}</strong></div>
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Hoy: <strong>${visitasHoy}</strong></div>
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Semana: <strong>${visitasSemana}</strong></div>
    </div>

    <!-- PORTFOLIO / Fiori Poetic Blog -->
    <div class="mt-1">
      <div class="pulsate-bck-normal ml-2 mr-2 mb-2 p-1 rounded-xl flex justify-center place-items-center mt-2 h-12 bg-slate-50/25">
        <img src="Screenshot_20251018_130052_TikTok.jpg" class="h-10 w-10 object-contain rounded-xl"/>
        <a href="https://81andie.github.io/my-portfolio/">
          <p class="text-2xl font-bold text-violet-100 text-center">PORTFOLIO</p>
        </a>
      </div>

      <div class="scale-up-bottom-normal ml-2 mr-2 p-1 mb-4 rounded-xl flex justify-center place-items-center mt-2 h-12 bg-slate-50/45">
        <img src="Galaxy-Fold2-localhost (8).png" class="h-10 w-10 rounded-xl object-contain"/>
        <a href="https://fiori-jxm6.vercel.app/#/inicio">
          <p class="text-2xl font-bold text-violet-850 text-center">Fiori Poetic Blog</p>
        </a>
      </div>
    </div>
  </div>

  <!-- Footer -->
  <div class="p-1 bottom-0 text-violet-950 bg-violet-100 text-xs text-md font-thin flex text-center flex-col justify-center justify-items-center">
    <span class="mt-1">Esta web ha sido diseñada y desarrollada por:<br> <strong>Andrea Hernández</strong></span>
    <span>Todos los derechos reservados<br> Copyright © 2025</span>
  </div>
`

setupCounter(document.querySelector('#counter'))*/


import './style.css';
import { setupCounter } from './counter.js';


const today = new Date();
const month = today.getMonth(); 
const day = today.getDate();

let backgroundImage = '/business-office.jpg'; 
let themeClass = ''; 


if (month === 9 && day >= 23 && day <= 31) {
  backgroundImage = '/halloween2.jpg';
  themeClass = 'halloween-mode';
}

else if (month === 11 && day >= 10 && day <= 31) {
  backgroundImage = '/christmas.jpg';
  themeClass = 'christmas-mode';
}

else if (month === 0 && day >= 1 && day <= 7) {
  backgroundImage = '/añonuevo.jpg';
  themeClass = 'newyear-mode';
}

else if ((month === 5 && day >= 15) || (month > 5 && month < 8) || (month === 7 && day <= 31)) {
  backgroundImage = '/verano.jpg';
  themeClass = 'summer-mode';
}

if (themeClass) document.body.classList.add(themeClass);

// --- CONTADORES DE VISITAS ---
const todayStr = today.toISOString().slice(0,10);
const dayOfWeek = today.getDay(); // 0=domingo
const monday = new Date(today);
monday.setDate(today.getDate() - ((dayOfWeek + 6) % 7));
const week = monday.toISOString().slice(0,10);

let visitasHoyData = JSON.parse(localStorage.getItem('visitasHoyData') || '{}');
let visitasSemanaData = JSON.parse(localStorage.getItem('visitasSemanaData') || '{}');

let totalVisits = parseInt(localStorage.getItem('totalVisits') || '0') + 1;
localStorage.setItem('totalVisits', totalVisits);

visitasHoyData[todayStr] = (visitasHoyData[todayStr] || 0) + 1;
visitasSemanaData[week] = (visitasSemanaData[week] || 0) + 1;

localStorage.setItem('visitasHoyData', JSON.stringify(visitasHoyData));
localStorage.setItem('visitasSemanaData', JSON.stringify(visitasSemanaData));

const visitasHoy = visitasHoyData[todayStr];
const visitasSemana = visitasSemanaData[week];

// --- Traducciones ---
const translations = {
  title: {
    es: "Programadora Web Frontend",
    ca: "Programadora Web Frontend",
    en: "Frontend Web Developer"
  },
  education: {
    es: "Licenciada en Pedagogía General de la Música\nConservatorio Superior del Liceo",
    ca: "Llicenciada en Pedagogia General de la Música\nConservatori Superior del Liceu",
    en: "Bachelor in General Music Pedagogy\nLiceo Superior Conservatory"
  },
  visits: {
    es: "Visitas",
    ca: "Visites",
    en: "Visits"
  }
};

// --- Idioma por defecto ---
let currentLang = 'es';

// --- HTML ---
document.querySelector('#app').innerHTML = `


  <div class=" bg-cover bg-center h-screen flex flex-col justify-start justify-items-start ${themeClass}" style="background-image: url(${backgroundImage});">
  <!-- Dropdown de idioma personalizado -->
    <div class=" flex ml-2 top-0 w-32 mt-1 mb-1 text-white font-semibold">
      <button id="dropdownButton" class=" bg-violet-500 p-1 text-left rounded-sm">${currentLang.toUpperCase()}</button>
      <ul id="dropdownOptions" class="absolute flex w-32 ml-12 gap-2  rounded-sm  z-50">
        <li class="p-1 hover:bg-violet-300 cursor-pointer" data-lang="es">Español</li>
        <li class="p-1 hover:bg-violet-300 cursor-pointer" data-lang="ca">Català</li>
        <li class="p-1 hover:bg-violet-300 cursor-pointer" data-lang="en">English</li>
      </ul>
    </div>
    <!-- Nombre -->
    <div class="rounded-sm flex flex-col text-center mt-12">
      <h1 class="bg-white/25 ml-2 mr-2 text-2xl p-1 mt-1 rounded-xl text-emerald-50 font-bold flex flex-col">Andrea Hernández</h1>
    </div>

  

    <!-- Títulos -->
    <h1 id="title" class="ml-2 mr-2 mt-3 text-xl bg-white/15 p-2 rounded-xl text-violet-50 text-center"><strong></strong></h1>
    <h1 id="education" class="ml-2 mr-2 mt-1 text-xs p-1 rounded-xl text-emerald-50 text-center"><strong></strong></h1>

    <!-- Enlaces sociales -->
    <div class="flex justify-center justify-items-center mt-2 mb-5">
      <!-- TikTok, GitHub, LinkedIn -->
      <a href="https://www.tiktok.com/@fioripoeticblog?lang=es-419">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          xmlns="http://www.w3.org/2000/svg" width="44px" height="44px" viewBox="0 0 48 48" fill="#ffffff">
          <path d="M33.5 12.7a9.3 9.3 0 0 1-3.2-7.1h-5.1v24a4.4 4.4 0 1 1-4.4-4.4 4.4 4.4 0 0 1 1.4.2v-5.2a9.6 9.6 0 0 0-1.4-.1 9.6 9.6 0 1 0 9.6 9.6v-12 a14.3 14.3 0 0 0 8.3 2.7V15a9.2 9.2 0 0 1-5.2-2.3z"/>
        </svg>
      </a>
      <a href="https://github.com/81andie">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          width="44px" height="44px" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" fill="#ffffff" stroke="#ffffff">
          <path d="M24,2.5a21.5,21.5,0,0,0-6.8,41.9c1.08.2,1.47-.46,1.47-1s0-1.86,0-3.65 c-6,1.3-7.24-2.88-7.24-2.88A5.7,5.7,0,0,0,9,33.68c-1.95-1.33.15-1.31.15-1.31 a4.52,4.52,0,0,1,3.29,2.22c1.92,3.29,5,2.34,6.26,1.79a4.61,4.61,0,0,1,1.37-2.88 c-4.78-.54-9.8-2.38-9.8-10.62a8.29,8.29,0,0,1,2.22-5.77,7.68,7.68,0,0,1,.21-5.69 s1.8-.58,5.91,2.2a20.46,20.46,0,0,1,10.76,0c4.11-2.78,5.91-2.2,5.91-2.2 a7.74,7.74,0,0,1,.21,5.69,8.28,8.28,0,0,1,2.21,5.77 c0,8.26-5,10.07-9.81,10.61a5.12,5.12,0,0,1,1.46,4 c0,2.87,0,5.19,0,5.9s.39,1.24,1.48,1A21.5,21.5,0,0,0,24,2.5"/>
        </svg>
      </a>
      <a href="https://www.linkedin.com/in/andrea-hern%C3%A0ndez-abbab9288">
        <svg class="hover:scale-90 hover:border hover:rounded-full hover:border-white transition-discrete"
          xmlns="http://www.w3.org/2000/svg" width="44px" height="44px" viewBox="0 0 48 48" fill="#ffffff">
          <path d="M12 19h5.3v17H12zM14.6 12a3.1 3.1 0 1 1 0 6.2 3.1 3.1 0 0 1 0-6.2zM20.6 19h5v2.3 h.1a5.5 5.5 0 0 1 5-2.8c5.3 0 6.3 3.5 6.3 8v9.5h-5.3v-8.4c0-2-.1-4.6-2.8-4.6 s-3.3 2.1-3.3 4.4v8.6h-5.3V19z"/>
        </svg>
      </a>
    </div>

    <!-- MINI BADGE CON TOTAL / HOY / SEMANA -->
    <span class="text-center text-white"><strong>${translations.visits[currentLang]}</strong></span>
    <div class="flex justify-center mt-2 space-x-1 text-white text-sm">
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Total: <strong>${totalVisits}</strong></div>
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Hoy: <strong>${visitasHoy}</strong></div>
      <div class="bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm animate-fadeIn">Semana: <strong>${visitasSemana}</strong></div>
    </div>


    <!-- PORTFOLIO / Fiori Poetic Blog -->
    <div class="mt-1 ">
      <div class="pulsate-bck-normal ml-2 mr-2 mb-2 p-1 rounded-xl flex justify-center place-items-center mt-2 h-12 bg-slate-50/25">
        <img src="Screenshot_20251018_130052_TikTok.jpg" class="h-10 w-10 object-contain rounded-xl"/>
        <a href="https://81andie.github.io/my-portfolio/">
          <p class="text-2xl font-bold text-violet-100 text-center">PORTFOLIO</p>
        </a>
      </div>

      <div class="scale-up-bottom-normal ml-2 mr-2 p-1  rounded-xl flex justify-center place-items-center mt-2 h-12 bg-slate-50/45">
        <img src="Galaxy-Fold2-localhost (8).png" class="h-10 w-10 rounded-xl object-contain"/>
        <a href="https://fiori-jxm6.vercel.app/#/inicio">
          <p class="text-2xl font-bold text-violet-850 text-center">Fiori Poetic Blog</p>
        </a>
      </div>
    </div>
  </div>
  </div>
  




  <!-- Footer -->
  <div class="p-1 bottom-0 text-violet-950 bg-violet-100 text-xs text-md font-thin flex text-center flex-col justify-center justify-items-center">
    <span id="footerLabel" class="mt-1">Esta web ha sido diseñada y desarrollada por:<br> <strong>Andrea Hernández</strong></span>
    <span>Copyright © 2025</span>
  </div>
`;

// --- Función para actualizar textos ---
function updateTexts() {
  document.getElementById('title').innerHTML = `<strong>${translations.title[currentLang]}</strong>`;
  document.getElementById('education').innerHTML = `<strong>${translations.education[currentLang].replace(/\n/g,'<br>')}</strong>`;
}

// --- Inicializa textos ---
updateTexts();

// --- Dropdown funcional ---
const btn = document.getElementById('dropdownButton');
const options = document.getElementById('dropdownOptions');

btn.addEventListener('click', () => {
  options.classList.toggle('hidden');
});

options.querySelectorAll('li').forEach(option => {
  option.addEventListener('click', (e) => {
    currentLang = e.target.dataset.lang;
    btn.textContent = currentLang.toUpperCase();
    updateTexts();
    options.classList.add('hidden');
  });
});

// --- Inicializa contador ---
setupCounter(document.querySelector('#counter'));


